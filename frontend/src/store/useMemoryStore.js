...
export const useMemoryStore = create((set, get) => ({
  ...
  fetchPendingAssertions: async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/review/assertions/pending?${params}`, {
  ...
  fetchGraphData: async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/vectors?visual_multiplier=${get().visualMultiplier}${pidQuery}`,
        { headers: get().getHeaders() }
      );
      const data = await res.json();
  ...
  saveLlmConfigs: async (newConfigs) => {
    const configsToSave =
      newConfigs && Array.isArray(newConfigs) ? newConfigs : get().llmConfigs.configs;
  ...
  testConfig: async (config) => {
    ...
    try {
      ...
      set((state) => ({
        testResults: { ...state.testResults, [id]: { status: "error", error: err.message } },
      }));
    } finally {
  ...
  fetchDataByTab: (tab) => {
    ...
    if (tabLower === "learning" || tabLower === "learnings" || tabLower === "assertions")
      get().fetchLearnings();
    ...
    else if (tabLower === "consolidation" || tabLower === "consolidations")
      get().fetchConsolidations();
