...
import { Database, Plus, Trash2, Key, Cpu, RefreshCw, Filter, Brain, Copy } from "lucide-react";
...
export default function DataSourceManager() {
  ...
  }, [subTab, fetchProjects, fetchSources, fetchKeys, fetchJobs]);
          ...
          <div className="grid lg:grid-cols-3 h-full divide-x lg:divide-slate-700 divide-y lg:divide-y-0 overflow-y-auto">
            <div className="lg:col-span-2 flex flex-col h-full overflow-hidden">
              <div className="p-5 border-b border-slate-700 bg-slate-800/40">
                <h3 className="text-base font-bold text-white flex items-center gap-2">
                  <Filter className="w-5 h-5 text-emerald-400 animate-pulse" /> Ephemeral Data Feed
                  Channels
                </h3>
              </div>
            ...
            <div className="p-6 bg-slate-900/30 flex flex-col h-full justify-between">
              <div>
                ...
                <form onSubmit={handleCreateSourceSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">
                      Polling Schedule (Optional Cron)
                    </label>
                    <input
                      type="text"
        ...
        {subTab === "jobs" && (
          <div className="flex flex-col h-full overflow-hidden">
            <div className="p-5 border-b border-slate-700 flex justify-between items-center bg-slate-800/40">
              ...
              <button
                onClick={() => fetchJobs()}
                className="p-2 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-white transition-colors flex items-center gap-1 text-xs font-semibold"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${jobsLoading ? "animate-spin" : ""}`} /> Refresh
                Status
              </button>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-700/40 text-sm">
                    {jobs.map((j, i) => (
                      <tr key={i} className="hover:bg-slate-700/15 transition-colors">
                        <td className="p-4">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] uppercase font-extrabold tracking-wider border ${
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
          <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <p className="text-xs text-slate-400 text-center mb-4 leading-relaxed">
              ⚠️ <strong>Crucial Security Notice:</strong> Copy this credential immediately. Under
              token safety standards, it will not be displayed again.
            </p>
            <div className="bg-slate-900 p-4 rounded-xl border border-slate-700 mb-5 font-mono text-xs break-all text-blue-200 font-bold select-all leading-normal flex items-center justify-between gap-3">
              ...
              <button
                ...
                className="text-slate-400 hover:text-white transition-colors shrink-0 p-1.5 hover:bg-slate-800 rounded-md"
            ...
            <button
              ...
              className="w-full bg-slate-700 hover:bg-slate-600 text-white font-bold py-2.5 rounded-xl transition-all active:scale-95 text-xs"
            >
              Done
            </button>
          </div>
