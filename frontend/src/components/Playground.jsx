...
import { Play, Filter, Cpu, Database, HelpCircle, Activity } from 'lucide-react';
                    ...
                    <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
                        <div className="text-center max-w-xl mx-auto mb-8 space-y-2">
                            ...
                            <p className="text-slate-400 text-sm leading-relaxed">
                                Inspect how the semantic router splits traffic. Verify deterministic heuristics, LTP activation, and when LLM generation cycles are fully bypassed.
                            </p>
                        </div>
                                ...
                                <div className="space-y-6 animate-in fade-in duration-300">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="bg-slate-900 p-4 rounded-xl border border-slate-750">
                                            <div className="text-[10px] text-slate-500 uppercase font-extrabold tracking-wider mb-1">Route Strategy Decided</div>
                                            <div className="text-lg font-mono font-bold text-purple-400 uppercase">{playgroundResult.strategy}</div>
                                        </div>
                                        <div className="bg-slate-900 p-4 rounded-xl border border-slate-750">
                                            <div className="text-[10px] text-slate-500 uppercase font-extrabold tracking-wider mb-1">Synaptic Relevance Score</div>
                                            <div className="text-lg font-mono font-bold text-blue-400 uppercase">
                                                {playgroundResult.score !== undefined ? `${(playgroundResult.score * 100).toFixed(0)}%` : 'N/A'}
                                            </div>
                                        </div>
                                    </div>

                                    <div className="bg-slate-900 p-5 rounded-xl border border-slate-750">
                                        <div className="text-[10px] text-slate-500 uppercase font-extrabold tracking-wider mb-2.5">Contextual Answer</div>
                                    ...
                                    {playgroundResult.sources && playgroundResult.sources.length > 0 && (
                                        <div className="bg-slate-900 p-5 rounded-xl border border-slate-750">
                                            <div className="text-[10px] text-slate-500 uppercase font-extrabold tracking-wider mb-3">Supporting Synaptic Nodes</div>
                                            <div className="space-y-2">
                                                {playgroundResult.sources.map((src, idx) => (
                                                    <div key={idx} className="bg-slate-950/60 p-3 rounded-lg border border-slate-800 text-xs text-slate-300 leading-relaxed font-mono">
                                                        {src}
                                                    </div>
                                                ))}
                                            </div>
