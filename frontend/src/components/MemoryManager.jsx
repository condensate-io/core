            ...
            <div className="p-5 border-b border-slate-700 flex justify-between items-center bg-slate-800/40">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() =>
                    selectedProjectId
                      ? manualTrigger(selectedProjectId, "condense")
                      : alert(
                          "Please select a project in the Dashboard first to condense its memories."
                        )
                  }
                ...
                {selectedIds.length > 0 && (
                  <button
                    onClick={() => bulkDelete("memory")}
                    className="bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all active:scale-95"
                  >
                    ...
                    <tr>
                      <th className="p-4 w-12">
                        <input
                          type="checkbox"
                          onChange={(e) =>
                            setSelectedIds(e.target.checked ? memories.map((m) => m.id) : [])
                          }
                  ...
                  <tbody className="divide-y divide-slate-700/40 text-sm">
                    {memories.map((m) => (
                      <tr key={m.id} className="hover:bg-slate-700/15 transition-colors">
                        ...
                        <td
                          className="p-4 text-slate-300 truncate max-w-xl font-medium"
                          title={m.content}
                        >
                          {m.content}
                        </td>
                        <td className="p-4 text-right flex justify-end gap-1">
                          <button
                            onClick={() => openEdit({ type: "memory", data: m })}
                            className="p-2 hover:bg-blue-500/15 text-blue-400 rounded-lg transition-colors"
                          >
                    ...
                    <tr>
                      <th className="p-4 w-12">
                        <input
                          type="checkbox"
                          onChange={(e) =>
                            setSelectedIds(e.target.checked ? learnings.map((l) => l.id) : [])
                          }
                          checked={selectedIds.length === learnings.length && learnings.length > 0}
                          className="rounded text-blue-600 bg-slate-900 border-slate-700"
                        />
                      </th>
                      ...
                      <tr key={l.id} className="hover:bg-slate-700/15 transition-colors">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2 flex-wrap font-medium">
                            <span className="text-blue-300 font-semibold px-2 py-0.5 bg-blue-500/10 rounded">
                              {l.subject_text}
                            </span>
                            <span className="text-slate-400 text-xs italic">{l.predicate}</span>
                            <span className="text-purple-300 font-semibold px-2 py-0.5 bg-purple-500/10 rounded">
                              {l.object_text}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="p-4 text-right flex justify-end gap-1">
                          <button
                            onClick={() => deleteItem("learning", l.id)}
                            className="p-2 hover:bg-red-500/15 text-red-400 rounded-lg transition-colors"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                    ...
                    <tr>
                      <th className="p-4 w-12">
                        <input
                          type="checkbox"
                          onChange={(e) =>
                            setSelectedIds(e.target.checked ? entities.map((ent) => ent.id) : [])
                          }
                  ...
                  <tbody className="divide-y divide-slate-700/40 text-sm">
                    {entities.map((e) => (
                      <tr key={e.id} className="hover:bg-slate-700/15 transition-colors">
                        ...
                        <td className="p-4">
                          <span className="px-2.5 py-0.5 bg-slate-700 rounded-md text-[10px] uppercase font-bold tracking-wider text-slate-400 border border-slate-600">
                            {e.type}
                          </span>
                        </td>
                        <td className="p-4 text-right flex justify-end gap-1">
                          <button
                            onClick={() => openEdit({ type: "entity", data: e })}
                            className="p-2 hover:bg-blue-500/15 text-blue-400 rounded-lg transition-colors"
                          >
                            <FileText className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => deleteItem("entity", e.id)}
                            className="p-2 hover:bg-red-500/15 text-red-400 rounded-lg transition-colors"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                    ...
                    <tr>
                      <th className="p-4 w-12">
                        <input
                          type="checkbox"
                          onChange={(e) =>
                            setSelectedIds(e.target.checked ? relations.map((r) => r.id) : [])
                          }
                          checked={selectedIds.length === relations.length && relations.length > 0}
                          className="rounded text-blue-600 bg-slate-900 border-slate-700"
                        />
                      </th>
                    ...
                    {relations.map((r) => (
                      <tr key={r.id} className="hover:bg-slate-700/15 transition-colors">
                        ...
                        <td className="p-4 font-mono text-xs text-slate-400">
                          {r.from_id.slice(0, 12)}...
                        </td>
                        <td className="p-4">
                          <span className="px-2.5 py-0.5 bg-purple-500/10 text-purple-300 rounded text-xs uppercase tracking-wide border border-purple-500/10 font-bold">
                            {r.relation_type}
                          </span>
                        </td>
                        <td className="p-4 font-mono text-xs text-slate-400">
                        ...
                        <td className="p-4 text-right flex justify-end gap-1">
                          <button
                            onClick={() => openEdit({ type: "relation", data: r })}
                            className="p-2 hover:bg-blue-500/15 text-blue-400 rounded-lg transition-colors"
                          >
                            <FileText className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => deleteItem("relation", r.id)}
                            className="p-2 hover:bg-red-500/15 text-red-400 rounded-lg transition-colors"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
        ...
        {subTab === "consolidations" && (
          <div className="flex flex-col h-full">
            <div className="p-5 border-b border-slate-700 flex justify-between items-center bg-slate-800/40 flex-shrink-0">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <Brain className="w-5 h-5 text-blue-400 animate-pulse" /> Higher-Order
                Meta-Learnings
              </h3>
              <button
                onClick={() =>
                  selectedProjectId
                    ? manualTrigger(selectedProjectId, "consolidate")
                    : alert(
                        "Please select a project in the Dashboard first to run a consolidation cycle."
                      )
                }
            ...
            {loadingStates.consolidations ? (
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {consolidations.map((c) => (
                  <div
                    key={c.id}
                    className="bg-slate-900/60 rounded-xl border border-slate-700/80 p-5 hover:border-blue-500/50 transition-all duration-300 group shadow-lg"
                  >
                    <div className="flex justify-between items-start mb-3">
                      <span className="text-[9px] font-extrabold text-slate-500 uppercase tracking-widest px-2 py-0.5 bg-slate-800 rounded border border-slate-700">
                      ...
                      <div className="flex gap-3">
                        ...
                        <div className="bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-800 flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-slate-400">
                            {c.evidence_count} Source Memories
                          </span>
                        </div>
                      </div>
                      <button
                        onClick={() => deleteItem("consolidation", c.id)}
                        className="opacity-0 group-hover:opacity-100 p-2 text-slate-500 hover:text-red-400 transition-all rounded-lg hover:bg-red-500/10"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
            ...
            <div className="p-5 border-b border-slate-700 flex justify-between items-center bg-slate-800/40">
              ...
              {pendingAssertions.length > 0 && (
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 rounded-lg text-xs font-bold flex items-center gap-2 shadow-lg shadow-emerald-950/40 transition-all active:scale-95 border border-emerald-500/20 text-white"
                >
            ...
            {reviewLoading ? (
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {pendingAssertions.map((a) => (
                  <div
                    key={a.id}
                    className="bg-slate-900/60 rounded-xl border border-slate-700/80 p-5 flex justify-between items-center hover:border-amber-500/40 transition-all duration-300 shadow-md"
                  >
                    <div className="space-y-3">
                      <div className="flex items-center gap-2 flex-wrap font-medium">
                        <span className="text-blue-300 font-bold px-2.5 py-0.5 bg-blue-500/10 rounded-lg text-sm">
                          {a.subject_text}
                        </span>
                        <span className="text-slate-400 text-xs italic font-bold">
                          {a.predicate}
                        </span>
                        <span className="text-purple-300 font-bold px-2.5 py-0.5 bg-purple-500/10 rounded-lg text-sm">
                          {a.object_text}
                        </span>
                      </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => approveAssertion(a.id)}
                        className="bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded-lg text-xs font-bold text-white transition-all active:scale-95"
                      >
                        Approve
                      </button>
                      <button
                        onClick={() => rejectAssertion(a.id)}
                        className="bg-red-800 hover:bg-red-700 px-4 py-2 rounded-lg text-xs font-bold text-white transition-all active:scale-95"
                      >
                        Reject
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
          <div className="bg-slate-800 w-full max-w-lg rounded-2xl border border-slate-700 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-5 border-t border-slate-700/60 bg-slate-800/40 flex justify-end gap-3">
              <button
                onClick={() => setEditItem(null)}
                className="bg-slate-700 hover:bg-slate-600 text-slate-300 font-bold py-2 px-5 rounded-lg text-xs transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleSaveEdit}
                className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-6 rounded-lg text-xs transition-all shadow-lg shadow-blue-900/30"
              >
                Save Changes
              </button>
            </div>
