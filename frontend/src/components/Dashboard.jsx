...
export default function Dashboard() {
    ...
    const cards = [
        { label: 'Episodic Items', val: stats.total_memories, color: 'text-slate-200 border-slate-700' },
        { label: 'Assertions', val: stats.total_learnings, color: 'text-emerald-300 border-slate-700' },
        { label: 'Consolidations', val: stats.total_consolidations ?? 0, color: 'text-blue-300 border-blue-900/50' },
        { label: 'Entities', val: stats.total_entities ?? 0, color: 'text-emerald-300 border-emerald-800/50' },
        { label: 'Relations', val: stats.total_relations ?? 0, color: 'text-purple-300 border-purple-800/50' },
        { label: 'Pending Review', val: stats.pending_review ?? 0, color: 'text-amber-300 border-amber-800/50' },
        ...
        <div className="flex flex-col h-full gap-6 animate-in fade-in duration-300 overflow-hidden">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 flex-shrink-0">
                {cards.map((card, i) => (
                    ...
                    >
                        <div className="text-slate-400 text-xs font-semibold tracking-wider uppercase mb-1.5">{card.label}</div>
