  ...
  const menuItems = [
    {
      ...
      desc: "Synaptic graph & overview",
    },
    {
      path: "/ledger",
      label: "Cognitive Ledger",
      icon: DbIcon,
      desc: "Episodic & semantic data",
      count: (stats.total_memories || 0) + (stats.total_learnings || 0),
    },
    {
      path: "/ingestion",
      label: "Ingestion Pipelines",
      icon: Radio,
      desc: "Projects, feeds & credentials",
    },
    {
      path: "/playground",
      label: "Playground Sandbox",
      icon: Play,
      desc: "Router & Condenser tests",
    },
    {
      path: "/settings",
      label: "System Settings",
      icon: Wrench,
      desc: "Engines & synaptic learning",
    },
  ];
    ...
    <div className="flex h-screen bg-slate-900 text-slate-100 font-sans overflow-hidden">
      ...
      <div className="w-72 bg-slate-800/90 border-r border-slate-700/80 p-5 flex flex-col flex-shrink-0 backdrop-blur-md relative z-10">
        <div className="flex items-center gap-3 mb-10 pl-2">
          ...
          <div>
            <h1 className="text-lg font-black tracking-wider bg-gradient-to-r from-blue-400 to-indigo-300 bg-clip-text text-transparent uppercase">
              Memory Server
            </h1>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
              Cognitive Core v0.8.0
            </span>
          </div>
        </div>
        ...
        <nav className="space-y-1.5 flex-1 overflow-y-auto pr-1">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 shrink-0 transition-transform duration-300 ${isActive ? "scale-110" : "group-hover:scale-105 text-slate-500"}`}
                />
                <div className="flex-1 text-left">
                  <div className="text-sm font-bold leading-none mb-1">{item.label}</div>
                  <div
                    className={`text-[10px] transition-colors leading-none ${isActive ? "text-blue-100/70" : "text-slate-500"}`}
                  >
                    {item.desc}
                  </div>
                </div>
