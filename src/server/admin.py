...
# --- Qdrant Dependency ---
# Used centralized dependency from above
...
def create_key(name: str, project_id: str, db: Session = Depends(get_db)):
    ...
    prefix = new_key[:12]
    api_key = ApiKey(key=hashed_key, prefix=prefix, name=name, project_id=pid)
    db.add(api_key)
    db.commit()
...
def delete_key(key: str, db: Session = Depends(get_db)):
    if len(key) <= 15:  # It's a prefix or masked key representation
        key_record = db.query(ApiKey).filter(ApiKey.prefix == key).first()
    else:
        # Full key passed, match prefix first to filter then verify
        prefix = key[:12]
        records = db.query(ApiKey).filter(ApiKey.prefix == prefix).all()
        key_record = None
        for r in records:
            if verify_key(key, r.key):
                key_record = r
                break
        if not key_record:
            # Fallback direct match for legacy
...
def delete_project(
    ...
    if project:
        ...
        try:
            ...
            logger.warning("Failed to delete project vectors from Qdrant: %s", e)
...
def get_system_config():
    ...
    return {"review_mode": os.getenv("REVIEW_MODE", "manual").lower()}
